package dev.shreyasayyengar.logininterface.database;
...
public class MySQL {
    ...
    public MySQL(String username, String password, String database, String host, int port) {
        ...
        try {
            openConnection();
            Logger.getAnonymousLogger().info("Database Connection Established & Successfully Connected!");
        } catch (SQLException x) {
            Logger.getAnonymousLogger().severe("[FAILURE] ------------------------- [FAILURE]");
            Logger.getAnonymousLogger().severe("There was a problem connecting to the MySQL database. Please double check that the information is correct, and that the MySQL server is online");
            Logger.getAnonymousLogger().severe("[FAILURE] ------------------------- [FAILURE]");
        }
    }
